...
struct ContentView: View
{
    ...
    @State private var show18Popup = false
    ...
    @State private var showAgeAlert = true//!UserDefaults.standard.bool(forKey: "is18Checked") // чтобы каждый раз спрашивал просто тру прописать
        ...
        NavigationStack {
            ZStack {
                ...
                VStack {
                    ...
                    {
                        Text("Нажмите на кнопку в углу экрана, чтобы выбрать трекер")
                            .font(.title2)
                ...
                if manager.showTrackersList
                {
                    TrackersListView(
                        ...
                        onDelete:
                            {
                                tracker in trackerToStop = tracker
                                showStopConfirmation = true
                            }
                    )
                    .zIndex(1)
                    .animation(.spring(response: 0.45, dampingFraction: 0.75),
                               value: manager.showTrackersList)
                }
                if showWelcomePopup
                {
                    WelcomePopupView(
                        usedPresets: manager.trackers.map
                        {
                            $0.preset
                        },
                        onPresetSelected:
                            {
                                preset in manager.addTracker(with: preset)
            ...
            .alert("Вы уверены?", isPresented: $showStopConfirmation, actions:
                ...
                Button("Нет", role: .cancel)
                {
                     ...
                     message:
                        {
                Text("Причина рецидива/окончания будет сохранена в истории. Продолжить?")
            })
            .sheet(isPresented: $showReasonSheet, onDismiss: { stopReasonText = ""
            })
            {
                VStack(spacing: 24)
                {
                    Text("Причина рецидива/окончания?")
                        .font(.title2)
                        .bold()
                    TextField("можно не заполнять",
                              text: $stopReasonText,
                              axis: .vertical)
